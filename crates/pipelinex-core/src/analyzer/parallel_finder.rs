...
pub fn find_parallelization_opportunities(dag: &PipelineDag) -> Vec<Finding> {
        ...
        for dep_idx in &deps {
            ...
            if is_likely_false_dependency(dep_job, job) {
                let savings = dep_job
                    ...
                    .min(job.estimated_duration_secs);
                findings.push(Finding {
                    severity: Severity::High,
                    category: FindingCategory::SerialBottleneck,
                    title: format!("'{}' depends on '{}' unnecessarily", job.id, dep_job.id),
                    description: format!(
    ...
    for job in dag.graph.node_weights() {
        if is_test_job(job) && job.matrix.is_none() && job.estimated_duration_secs > 300.0 {
            ...
            let savings =
                job.estimated_duration_secs - (job.estimated_duration_secs / optimal_shards as f64);
...
fn is_likely_false_dependency(
    dep: &crate::parser::dag::JobNode,
    dependent: &crate::parser::dag::JobNode,
) -> bool {
    ...
    if dep_type == JobType::Lint
        && (dependent_type == JobType::Test || dependent_type == JobType::Build)
    {
        return true;
    }
...
fn classify_job(job: &crate::parser::dag::JobNode) -> JobType {
    ...
    let is_lint = name.contains("lint")
        || name.contains("format")
        || name.contains("style")
        || job_name.contains("lint")
        || job_name.contains("format");
    ...
    let is_deploy =
        name.contains("deploy") || name.contains("release") || job_name.contains("deploy");

    // Also check step contents
    let step_text: String = job
        .steps
        .iter()
    ...
    if is_lint
        || step_text.contains("eslint")
        || step_text.contains("clippy")
        || step_text.contains("prettier")
    {
        JobType::Lint
    } else if is_test
        || step_text.contains("pytest")
        || step_text.contains("jest")
        || step_text.contains("npm test")
    {
        JobType::Test
...
"#;
        ...
        assert!(findings
            .iter()
            .any(|f| matches!(f.category, FindingCategory::SerialBottleneck)));
    }
}
